"""Tiêu chí nghiệm thu của CLAUDE.md cho state machine Catalog.

Mỗi test ở đây map 1-1 với một tiêu chí trong đặc tả và CHẠY THẬT logic của
``wfx_panel/automation/catalog.py`` trên một DOM giả (``tests/fakes/wfx_dom.py``),
thay cho kiểu assert chuỗi trong source vốn xanh cả khi hành vi đã sai.

State machine bắt buộc:

    CATEGORY_CONFIRMED -> MASTER -> GRID_DATA_SETTLED -> FILTER_VISIBLE
    -> FILTER_VALUE_CONFIRMED -> FILTER_RESULTS_SETTLED -> 0/1/nhiều Code
"""
