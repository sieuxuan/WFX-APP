...
def test_public_version_is_1_0_13():
    assert APP_VERSION == "1.0.13"
    assert DISPLAY_VERSION == "1.0.13"
    assert RELEASE_TAG == "v1.0.13"
...
def test_windows_build_embeds_release_version():
    ...
    assert "ProductVersion', '1.0.13" in info
    assert "prodvers=(1, 0, 13, 0)" in info
