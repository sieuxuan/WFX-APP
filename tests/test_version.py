...
def test_public_version_is_1_0_18():
    assert APP_VERSION == "1.0.18"
    assert DISPLAY_VERSION == "1.0.18"
    assert RELEASE_TAG == "v1.0.18"
...
def test_windows_build_embeds_release_version():
    ...
    assert "ProductVersion', '1.0.18" in info
    assert "prodvers=(1, 0, 18, 0)" in info
