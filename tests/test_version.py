...
def test_public_version_is_1_0_30():
    assert APP_VERSION == "1.0.30"
    assert DISPLAY_VERSION == "1.0.30"
    assert RELEASE_TAG == "v1.0.30"
...
def test_windows_build_embeds_release_version():
    ...
    assert "ProductVersion', '1.0.30" in info
    assert "prodvers=(1, 0, 30, 0)" in info
