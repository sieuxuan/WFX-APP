...
def test_public_version_is_1_0_26():
    assert APP_VERSION == "1.0.26"
    assert DISPLAY_VERSION == "1.0.26"
    assert RELEASE_TAG == "v1.0.26"
...
def test_windows_build_embeds_release_version():
    ...
    assert "ProductVersion', '1.0.26" in info
    assert "prodvers=(1, 0, 26, 0)" in info
