...
- `wfx_panel/panel_app.py` + `wfx_panel/app/`: vỏ desktop — cửa sổ pywebview,
  launcher, tray, hotkey, hộp thoại file và cửa sổ Hướng dẫn.
...
- `wfx_panel/workbooks/` và `wfx_panel/stores/`: Python thuần, không chạm
  Playwright — luật file Excel và dữ liệu tham chiếu, test được không cần Chrome.
- `wfx_panel/ui/`: HTML/CSS/JavaScript của panel; `ui/panel/` là 17 script chia
  chung global scope, nạp theo đúng thứ tự `index.html` khai báo.
...
python -m pytest
ruff check .
Get-ChildItem wfx_panel/ui/panel/*.js | ForEach-Object { node --check $_ }
```
...
- `WFX-Smart-Setup-v1.0.34.exe` — bản khuyên dùng. Cài theo user, không cần
  quyền Administrator, tự tạo shortcut Desktop và Start Menu, có Uninstall và
  nâng cấp tại chỗ.
- `WFX-Smart-v1.0.34-win64.zip` — bản portable. Giải nén nguyên thư mục rồi mở
